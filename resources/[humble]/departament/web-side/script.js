    ...
    for (let i = 0; i < items[category].length; i++) {
        ...
        $(".items").append(
            `<button class='item' onclick='selectItem("`+imagem+`","`+nome+`","`+descricao+`","`+item+`","`+preco+`")'>`+
                `<img src='https://cdn.humblegg.com/host-page/inventory-fivem/`+imagem+`.png' alt=''>`+nome+
            `</button>`
        );
    }
...
function selectItem(imagem,nome,descricao,item,preco) {
    ...
    $(".item-image").html("<img src='https://cdn.humblegg.com/host-page/inventory-fivem/"+imagem+".png' alt=''>");
