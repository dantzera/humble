...
function vRP.CharacterChosen(source,Passport,Model)
    ...
    if not Characters[source] then
        ...
        PerformHttpRequest(HumbleBot["Welcome"],function(source,Passport,Model)end,"POST",json.encode({ username = ServerName,content = Account["discord"].." "..Passport.." "..Query[1]["name"].." "..Query[1]["name2"]}),{["Content-Type"] = "application/json" })
    end
