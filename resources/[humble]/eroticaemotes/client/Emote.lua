        ...
        while AnimationThreadStatus and (IsInAnimation or PtfxPrompt) do
            ...
            if PtfxPrompt then
                ...
                if IsControlPressed(0, 46) then
                    ...
                    Wait(PtfxWait)
                    if PtfxCanHold then
                        while IsControlPressed(0, 46) and IsInAnimation and AnimationThreadStatus do
                            Wait(5)
                        end
                    end
                    PtfxStop()
...
if Config.EnablekeybindtoCancel then
    RegisterKeyMapping("emotecancel", "Cancel current emote", "keyboard", Config.Keybindtocancel)
end
...
RegisterCommand('e2', function(source, args, raw) EmoteCommandStart(source, args, raw) end)
    ...
    if GetEntityHealth(PlayerPedId()) > 101 then
        if #args > 0 then
            ...
            elseif DP.PropEmotes[name] ~= nil then
                ...
                SimpleNotify("'" .. name .. "' " .. Config.Languages[lang]['notvalidemote'] .. "")
            end
        end
    end    
end
