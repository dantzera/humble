$(document).ready(function(){
	window.addEventListener("message",function(event){
		switch(event["data"]["Action"]){
			...
			case "Update":
				$("#firstPartWeightText").html(`${(event["data"]["invPeso"]).toFixed(2)}   /   ${(event["data"]["invMaxpeso"]).toFixed(2)}`);
				$("#secondPartWeightText").html(`${(event["data"]["chestPeso"]).toFixed(2)}   /   ${(event["data"]["chestMaxpeso"]).toFixed(2)}`);
...
const updateDrag = () => {
	...
	$(".populated").tooltip({
		create: function(event,ui){
			...
			$(this).tooltip({
				content: `<item>${name}</item>${description !== "false" ? "<br><description>"+description+"</description>":""}<br><legenda>Economia: <r>$${economy}</r> <s>|</s> Máximo: <r>${max !== "undefined" ? max:"S/L"}</r>${serial !== undefined ? ` <s>|</s> Serial: <r>${serial}</r>` : ""}</legenda>`,
...
const Chest = () => {
	$.post("http://chest/Chest",JSON.stringify({}),(data) => {
		$("#firstPartWeightText").html(`${(data["invPeso"]).toFixed(2)}   /   ${(data["invMaxpeso"]).toFixed(2)}`);
		$("#secondPartWeightText").html(`${(data["chestPeso"]).toFixed(2)}   /   ${(data["chestMaxpeso"]).toFixed(2)}`);
		...
		for (let x = 1; x <= data["invMaxpeso"]; x++){
			...
			if (data.Inventory[slot] !== undefined){
				...
				const item = `<div class="item populated" title="" style="filter: saturate(350%);" data-max="${v["max"]}" data-economy="${v["economy"]}" data-description="${v["desc"]}" data-serial="${v["serial"]}" data-amount="${v.amount}" data-peso="${v.peso}" data-item-key="${v.key}" data-name-key="${v.name}" data-slot="${slot}">
					<div class="top">
						<img class="image-item" src="https://cdn.humblegg.com/host-page/inventory-fivem/${v["index"]}.png" style="width: 60px; height: 60px;" />
						<div class="itemWeight">${(v.peso*v.amount).toFixed(2)}</div>
						<div class="itemAmount">${formatarNumero(v.amount)}x</div>
					</div>

					<div class="durability" style="width: ${actualPercent == 1 ? "100":actualPercent}%; background: ${actualPercent == 1 ? "#ff0055":colorPicker(actualPercent)};"></div>
				</div>`;
