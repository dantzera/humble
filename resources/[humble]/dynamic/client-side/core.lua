...
RegisterCommand("globalFunctions",function()
	if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not menuOpen and LocalPlayer["state"]["Route"] < 900000 and not IsPauseMenuActive() then
		...
		if GetEntityHealth(Ped) > 100 then
			...
			exports["dynamic"]:AddButton("Informações","Informações pessoais.","inventory:weight","","others",true)
...
RegisterCommand("tencodeFunctions",function()
	if (LocalPlayer["state"]["Police"] or LocalPlayer["state"]["Comanf"]) and not IsPauseMenuActive() then
		if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not menuOpen and LocalPlayer["state"]["Route"] < 900000 then
			...
			if LocalPlayer["state"]["Police"] or LocalPlayer["state"]["Comanf"] then
				exports["dynamic"]:AddButton("QTI","Deslocamento.","dynamic:TencodeQTI","1",false,true)
				exports["dynamic"]:AddButton("QTH","Localização.","dynamic:TencodeQTH","2",false,true)
				exports["dynamic"]:AddButton("QRR","Apoio com prioridade.","dynamic:TencodeQRR","3",false,true)
				exports["dynamic"]:AddButton("QRT","Oficial desmaiado/ferido.","dynamic:TencodeQRT","4",false,true)
				exports["dynamic"]:AddButton("CD4","Área Limpa","dynamic:TencodeCD4","5",false,true)
			end
...
RegisterCommand("emergencyFunctions",function()
	if (LocalPlayer["state"]["Police"] or LocalPlayer["state"]["Comanf"] or LocalPlayer["state"]["Paramedic"]) and not IsPauseMenuActive() then
		if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not menuOpen and LocalPlayer["state"]["Route"] < 900000 then
			...
			if GetEntityHealth(Ped) > 100 then
				if not IsPedInAnyVehicle(Ped) then
					...
					if LocalPlayer["state"]["Police"] or LocalPlayer["state"]["Comanf"] then
						exports["dynamic"]:AddButton("Computador","Computador de bordo policial.","police:MdtHumble","",false,false)
						exports["dynamic"]:AddButton("Invadir","Invadir a residência.","propertys:invadeSystem","",false,false)
					end
				end
			end
